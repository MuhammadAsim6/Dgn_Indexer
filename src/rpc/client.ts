...
export function createRpcClient(opts: RpcClientOptions): RpcClient {
  ...
  async function fetchDenomTrace(hash: string): Promise<any> {
    ...
    // Try REST first (gRPC-gateway: /ibc/apps/transfer/v1/denoms/{hash})
    try {
      const j = await getJson<any>(`/ibc/apps/transfer/v1/denoms/${hex}`);
      return j.denom_trace ?? j.denom ?? j;
    } catch (restErr) {
      log.debug(`[rpc] REST denom lookup failed for ${hash}, falling back to ABCI`, String(restErr));
    }

    // ABCI fallback — return the raw value for the caller to decode
    try {
      const res = await queryAbci('/ibc.applications.transfer.v1.Query/DenomTrace', hex);
      if (res && res.value) {
        // Return the base64-encoded protobuf value; caller must decode
        return { _abciValue: res.value, hash: hex };
      }
    } catch (abciErr) {
      log.debug(`[rpc] ABCI denom lookup also failed for ${hash}`, String(abciErr));
    }

    throw new Error(`fetchDenomTrace: both REST and ABCI failed for ${hash}`);
  }
