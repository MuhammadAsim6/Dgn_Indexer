  ...
  async function processHeight(h: number) {
    try {
      ...
      try {
        ...
        // ✅ FIX: If sink.write() fails inside tryFlush, don't let it become
        // an unhandled rejection that stalls the pipeline. Log and re-queue.
        log.error(`[syncRange] tryFlush failed at height ${h}: ${flushErr?.message ?? flushErr}`);
        // The block is still in 'ready' buffer — it will be flushed next time
        // tryFlush is called from a subsequent height's completion.
      }
    }
  }
